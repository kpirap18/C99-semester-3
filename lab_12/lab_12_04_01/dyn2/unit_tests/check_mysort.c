...
START_TEST(tests_arr_one_sort)
{
    ...
    mysort(pa, 1, sizeof(int), comparator_int);
    ck_assert_int_eq(*pa, 1);
}
END_TEST

START_TEST(tests_usual_sort_int)
{
    int *pa;
    int a[5] = {4, -9, 1, 0, -7};
    pa = a;

    mysort(pa, 5, sizeof(int), comparator_int);
    ck_assert_int_eq(*pa, -9);
    ck_assert_int_eq(*(pa + 1), -7);
    ck_assert_int_eq(*(pa + 2), 0);
    ck_assert_int_eq(*(pa + 3), 1);
    ck_assert_int_eq(*(pa + 4), 4);
}
END_TEST

START_TEST(tests_usual_sort_char)
{
    char a[5] = "bcakf";
...
START_TEST(tests_pos_dsort2)
{
    double a[5] = {9, 8.4, 7.858, 5, 1.8};
...
Suite* mysort_suite(void)
{
	...
	mysort = (func_mysort)GetProcAddress(hlib, "mysort");
    ...
    tcase_add_test(tc_pos, tests_usual_sort_int);
    tcase_add_test(tc_pos, tests_usual_sort_char);
    tcase_add_test(tc_pos, tests_neg_sort);
    tcase_add_test(tc_pos, tests_pos_sort);
    tcase_add_test(tc_pos, tests_neg_sort2);
    tcase_add_test(tc_pos, tests_pos_sort2);
    tcase_add_test(tc_pos, tests_pos_dsort);
    tcase_add_test(tc_pos, tests_pos_dsort2);
