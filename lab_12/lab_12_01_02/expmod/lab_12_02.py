...
''' Для задачи номер 1: Сдвиг массива на k позиций

    arr - массив для сдвига
    k - на сколько надо сдвинуть
    text - куда записывать информацию
'''
def task_01(arr, k, text):
    new_arr = libarray_dll.shift_arr(arr, len(arr), k)
    wr_label(new_arr, text)
...
def task_02(rang, text):
    choice = sel.get()
    if (choice == 1):
        sq_arr_n = libarray_dll.square_arr(list_get(rang),
                                           len(list_get(rang)),
                                           len(list_get(rang)))
        rc = sq_arr_n[0]
        sq_arr = sq_arr_n[1]
...
''' Создание окна пиложения и необходимого меню '''
mainmenu = Menu(roots)
...
filemenu1.add_command(label = 'TASK №1',
                      command = lambda: task_01(list_get(n1_entry),
                                                k_get(n12_entry),
                                                t1_out))
